...
loadSTM<-function(x) { 
  ...
  print(paste("Loaded experiment ",x,", which has reads for",days,"days"));
...
for (m in 1:dims[1]){
  for (d in 1:dims[2]){
    ...
    pracvar[m,d,g]=FullCombinedVarianceThisRatio 
...
#This function performs inverse-variance weighted mean calculations
 ...
 if(is.na(var1)){var1<-0}
  var<-max(var1,var2) #to be conservative take the maximum of two different variance measures.
  return(c(mean,var))
}
...
for (g in 1:dims[3]){
  ...
  if(length(minfitnessloc)==1 & !(is.na(minvariance))){
   if(variances[minfitnessloc]==minvariance){ # this heuristic avoids one very 'confident' result indicating essentiality overpowering the rest of the data
...
#The following section calculates a number of estimates for the normalised relationship between the barcode ratio in the input and that in the population on day 6. In the case of dispensable genes these are measurements of integration efficiency.
currentarget=which(filteredgenes=="PBANKA_103780")
if("PBANKA_103780"%in% filteredgenes){
normd6toinputA=d6toinput/d6toinput[currentarget]
normd7toinputA=d7toinput/d7toinput[currentarget]
norminput=filteredinputratio/filteredinputratio[currentarget]
}
else{
...
currentarget=which(filteredgenes=="PBANKA_051490")
if("PBANKA_051490"%in% filteredgenes){
normd6toinputB=d6toinput/d6toinput[currentarget]
}
else{
normd6toinputB <- rep(NA, dims[3])
}

currentarget=which(filteredgenes=="PBANKA_051500")
if("PBANKA_051500"%in% filteredgenes){
normd6toinputC=d6toinput/d6toinput[currentarget]
}
else{
normd6toinputC <- rep(NA, dims[3])
}

currentarget=which(filteredgenes=="p230p-tag")
if("p230p-tag"%in% filteredgenes){
normd6toinputD=d6toinput/d6toinput[currentarget]
}
else{
...
for (d in 1:(dims[2]-1)){
  ...
  merge$variance=merge$variance*3 # This factor is necessary for agreement with measured reproducibility
...
#This function simulates the variance produced by 45 cycles of PCR with an efficiency of 0.8 by repeatedly simulating the procedure for a range of input DNA concentrations and observing the resultant distribution, it caches the results for future use
